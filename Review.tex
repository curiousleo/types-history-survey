...
\abstract{%
In order to make automated reasoning possible, we need to represent our propositions and proofs in a way that computers can read and manipulate. Lambda calculi have been used to build such representations with great success.
...
\begin{prooftree}
    ...
    \LeftLabel{(pair)}
    \TrinaryInfC{\(\Gamma \vdash \texttt{pair}_{\Sigma x:A.B} (M,N) : (\Sigma x:A.B)\)}
\end{prooftree}
